...
- [x] **Deployment**: public scenario and comparison responses contain only aggregated metrics, explanations, observed competitor references, and optional aggregated model analysis. CSV files and survey rows have no public route. Vercel applies a same-origin content security policy plus `nosniff`, frame-denial, and referrer-policy headers. On 2026-09-17, Production was verified at the same commit as `origin/main`, with the required headers, favicon, robots file, and scenario API response. The stable public deployment is [lumen-g27.vercel.app](https://lumen-g27.vercel.app).
- [x] **Files generated along the way**: this prototype creates no committed files derived from the provided case data. Test fixtures use only synthetic sentinel values.
...
- [x] **Robustness**: browser inputs restrict price to the observed EUR 0.62–3.09 support and payback horizon to 0.01–120 months. The API and direct calculation path enforce the same bounds, a maximum of three scenarios, and the selected baseline; invalid input receives a safe 422 response without echoing it. Channel comparison always includes each official channel once. The browser shows safe business-readable messages for invalid, unavailable, malformed, non-JSON, timed-out, or network-failed responses, including a public validation reason when supplied.
- [x] **Explainability**: each decision metric shows its business meaning, status, threshold comparison where approved, and an expandable explanation of formula, source, assumptions, and limits. The decision driver has a business label, threshold context, and a stated selection rule; internal keys remain API compatibility fields only. The browser renders backend-provided metadata and does not calculate business logic.
...
LUMEN needs a defensible German launch scenario, not a generic calculator. Our tool lets a manager test a retail price, sales channel, launch month, and payback horizon. It combines channel economics, customer price acceptability, seasonality, and lifetime value to customer acquisition cost into one transparent GO, CONDITIONAL, or NO-GO verdict. It also shows the same scenario through CMO and CFO perspectives, without creating competing verdicts. A non-recoverable payback remains a nullable numeric API value and is shown to managers as “Not recoverable”, never as a technical infinity value. On request, the selected baseline also shows model price sensitivity and launch-month context. A manager can print the evaluated inputs, decision, trade-off, metrics, sources, timestamp, and page URL through the browser print dialog. We deliberately show the trade-off between customer reach and financial return, exclude personal data, and publish only aggregated results.
